...
import static com.github.mikephil.charting.data.LineDataSet.Mode.LINEAR;
...
public class MPChartHelper {
    ...
    public static void setLinesChart(LineChart lineChart, List<String> xAxisValue, List<List<Float>> yXAxisValues, List<String> titles, boolean showSetValues, int[] lineColors) {
        ...
        xAxis.setValueFormatter(xAxisFormatter);
//        xAxis.setAxisMaximum(Float.parseFloat(xAxisValue.get(xAxisValue.size()-1)));
//        xAxis.setAxisMinimum(Float.parseFloat(xAxisValue.get(0)));
        //y轴设置
    ...
    private static void setLinesChartData(LineChart lineChart, List<List<Float>> yXAxisValues, List<String> titles, boolean showSetValues, int[] lineColors) {
        ...
        if (lineChart.getData() != null && lineChart.getData().getDataSetCount() > 0) {
            ...
            for (int i = 0; i < entriesList.size(); ++i) {
                ...
                set.setDrawValues(false);//在点上显示数值 默认true
                set.setDrawCircles(false);//在点上画圆 默认true
                set.setHighLightColor(Color.parseColor("#EBE4F8"));
                set.setDrawFilled(true);
                set.setColor(Color.parseColor("#46ADF0"));
                set.setFillColor(Color.parseColor("#46ADF0"));
                set.setFillAlpha(40);
                dataSets.add(set);
            }
